...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#themtk").on("hide.bs.modal", function () {
            $('#themtk').find('.modal-body').html('');
        });
...
<center>
    <table class="table table-hover" width="100%" style="text-align:left">
        ...
        <tr>
            ...
            <td>
                 <input type="button" class="btn btn-primary" value="Đổi mật khẩu" onclick="SuaTrenModal(<%= item.ID %>)" /> &nbsp;&nbsp;|&nbsp;&nbsp; 
                 <input type="button" class="btn btn-primary" value="Xóa tài khoản" onclick="XoaTaiKhoan(<%= item.ID %>)" />
            </td>
        </tr>
        <% } %>
    </table>
